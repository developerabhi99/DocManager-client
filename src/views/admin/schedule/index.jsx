...
export default function DoctorSchedule() {
  ...
  const fetchDoctors = useCallback(async () => {
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        // Filter for doctors (users with "DOCTOR" in their role name)
        const doctorUsers = data.users?.filter(user => 
          user.role?.name?.toLowerCase().includes('doctor')
        ) || [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/doctors/${selectedDoctor}/schedules`, {
        method: 'POST',
        headers: {
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${API_BASE}/doctors/${selectedDoctor}/schedules/${scheduleToDelete}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <Box pt={{ base: '130px', md: '80px' }} px={6}>
      <VStack spacing={6} align="stretch">
        <Heading>Doctor Schedule Management</Heading>

        {/* Doctor Selection (for admins) */}
        {canManageOtherDoctors && (
          <Card maxW="400px">
            <CardBody>
              <FormControl>
        ...
        {selectedDoctor && (
          <>
            ...
            <Card maxW="500px">
              <CardHeader>
                <Heading size="md">
                  {isEditing ? 'Edit Schedule' : 'Add Schedule'} - {currentDoctorName}
                </Heading>
              </CardHeader>
              <CardBody>
